...
static constexpr std::uint8_t CaseInsensitiveMap[256] = {
...
static constexpr auto caseInsensitiveRead64(const std::uint8_t *position) noexcept
    -> std::uint64_t {
    return (static_cast<std::uint64_t>(CaseInsensitiveMap[position[0]]) << 0) |
           (static_cast<std::uint64_t>(CaseInsensitiveMap[position[1]]) << 8) |
           (static_cast<std::uint64_t>(CaseInsensitiveMap[position[2]]) << 16) |
           (static_cast<std::uint64_t>(CaseInsensitiveMap[position[3]]) << 24) |
           (static_cast<std::uint64_t>(CaseInsensitiveMap[position[4]]) << 32) |
           (static_cast<std::uint64_t>(CaseInsensitiveMap[position[5]]) << 40) |
           (static_cast<std::uint64_t>(CaseInsensitiveMap[position[6]]) << 48) |
           (static_cast<std::uint64_t>(CaseInsensitiveMap[position[7]]) << 56);
}
...
/// \brief
///   Rapid hash implementation for case-insensitive string.
[[nodiscard]]
static auto caseInsensitiveRapidHash(const void *data, std::size_t size) noexcept -> std::uint64_t {
    std::uint64_t seed =
    ...
    if (size <= 16) [[likely]] {
        if (size >= 4) [[likely]] {
            ...
            a = static_cast<std::uint64_t>(CaseInsensitiveMap[p[0]]) << 56;
            a |= static_cast<std::uint64_t>(CaseInsensitiveMap[p[size >> 1]]) << 32;
            a |= static_cast<std::uint64_t>(CaseInsensitiveMap[p[size - 1]]);
            b = 0;
        } else {
            a = 0;
            b = 0;
        ...
        if (i > 48) {
            ...
            while (i >= 96) [[likely]] {
                seed  = rapidHashMix(caseInsensitiveRead64(p) ^ RapidHashSecret[0],
                                     caseInsensitiveRead64(p + 8) ^ seed);
                seed1 = rapidHashMix(caseInsensitiveRead64(p + 16) ^ RapidHashSecret[1],
                                     caseInsensitiveRead64(p + 24) ^ seed1);
                seed2 = rapidHashMix(caseInsensitiveRead64(p + 32) ^ RapidHashSecret[2],
                                     caseInsensitiveRead64(p + 40) ^ seed2);
                seed  = rapidHashMix(caseInsensitiveRead64(p + 48) ^ RapidHashSecret[0],
                                     caseInsensitiveRead64(p + 56) ^ seed);
                seed1 = rapidHashMix(caseInsensitiveRead64(p + 64) ^ RapidHashSecret[1],
                                     caseInsensitiveRead64(p + 72) ^ seed1);
                seed2 = rapidHashMix(caseInsensitiveRead64(p + 80) ^ RapidHashSecret[2],
                                     caseInsensitiveRead64(p + 88) ^ seed2);
                p += 96;
                i -= 96;
            }
        ...
        if (i > 16) {
            seed = rapidHashMix(caseInsensitiveRead64(p) ^ RapidHashSecret[2],
                                caseInsensitiveRead64(p + 8) ^ seed ^ RapidHashSecret[1]);
            if (i > 32) {
                seed = rapidHashMix(caseInsensitiveRead64(p + 16) ^ RapidHashSecret[2],
                                    caseInsensitiveRead64(p + 24) ^ seed);
            }
        }
...
auto CaseInsensitiveStringHash::operator()(argument_type value) const noexcept -> result_type {
    // Curretly we truncate the hash value directly for 32-bit operating systems.
    return static_cast<result_type>(caseInsensitiveRapidHash(value.data(), value.size()));
}
...
auto CaseInsensitiveStringEqual::operator()(argument_type lhs, argument_type rhs) const noexcept
    ...
    return std::ranges::equal(lhs, rhs, [](char a, char b) {
        return CaseInsensitiveMap[static_cast<std::uint8_t>(a)] ==
               CaseInsensitiveMap[static_cast<std::uint8_t>(b)];
    });
}
...
/// \brief
///   Result for unsigned integer parser.
...
/// \tparam Integer
///   Integer type to parse.
/// \tparam Iterator
///   Iterator to the first character of the input range.
/// \param first
///   Iterator to the first character of the input range.
/// \param last
///   Iterator to the end of the input range.
/// \return
///   A pair of iterator to the next character after the parsed integer and the parsed integer.
template <typename Integer, std::random_access_iterator Iterator>
[[nodiscard]] static auto parseUnsignedInt(Iterator first, Iterator last) noexcept
    -> UIntParseResult<Integer, Iterator> {
    Integer value = 0;
...
/// \tparam Iterator
///   Iterator type of the input range.
/// \param first
///   Iterator to the first character of the input range.
/// \param last
///   Iterator to the end of the input range.
/// \return
///   A pair of iterator to the next character after the parsed date and the parsed date.
template <std::random_access_iterator Iterator>
[[nodiscard]] static auto parseHttpDate(Iterator first, Iterator last) noexcept
    -> HttpDateParseResult<Iterator> {
    ...
    if (first != last && *first == ',') [[likely]]
        ...
        return {first, {}}; // Invalid date string.
    ...
    std::chrono::system_clock::time_point sysTime{sysDays + hour + minute + second};
...
auto HttpHeaders::contentLength() const noexcept -> std::optional<std::size_t> {
    ...
    auto result = parseUnsignedInt<std::size_t>(value.begin(), value.end());
    return result.value;
}
...
auto HttpHeaders::date() const noexcept -> std::optional<std::chrono::system_clock::time_point> {
    ...
    auto result = parseHttpDate(value.begin(), value.end());
    // FIXME: Should we use 1970-01-01 as error date?
    if (result.time == std::chrono::system_clock::time_point{}) [[unlikely]]
        return std::nullopt;
...
ServiceCollection::ServiceCollection() noexcept : m_services{31} {}

ServiceCollection::ServiceCollection(ServiceCollection &&other) noexcept
    : m_services{std::move(other.m_services)} {}

ServiceCollection::~ServiceCollection() noexcept {
    // Destroy all services.
    for (auto kv : m_services) {
        ServiceWrapper &wrapper = kv.second;
        wrapper.destroy(wrapper.service);
    }
}

auto ServiceCollection::operator=(ServiceCollection &&other) noexcept -> ServiceCollection & {

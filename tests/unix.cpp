...
TEST_CASE("[UnixListener] Unix ping-pong") {
    ...
    auto server = [&](UnixStream stream) -> Task<> {
    ...
    auto listener = [&](UnixSocketAddress address) -> Task<> {
        UnixListener srv;
    ...
    auto client = [&](UnixSocketAddress address) -> Task<> {
        UnixStream stream;
    ...
    UnixSocketAddress address{"onion-test-pingpong.sock"};
